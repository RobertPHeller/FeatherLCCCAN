 ...
 */
Adafruit_TSC2007::Adafruit_TSC2007(void) : i2c_(-1) {}

Adafruit_TSC2007::~Adafruit_TSC2007(void) {
  if (i2c_ >= 0) {
    ::close(i2c_); // close i2c device
  }
}
 ...
 */
bool Adafruit_TSC2007::begin(uint8_t address, const char * name) {
  if (i2c_>=0) {
    ::close(i2c_); // remove old interface
  }
    
    i2c_ = ::open(name, O_RDWR);
    if (i2c_ < 0) return false;
    ::ioctl(i2c_, I2C_SLAVE, address);


  /*
  uint8_t setupcmd = 0b10110000;
  if (::write(i2c_, &setupcmd, 1) < 1) {
    return false;
  }
  */
